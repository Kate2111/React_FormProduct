...
export const productGroupSlice = createSlice({
  ...
  reducers: {
    ...
    removeGroup: (state, action: PayloadAction<string | number>) => {
      state.productGroup = state.productGroup.filter((group) => group.id !== action.payload);
    },
    addProduct: (state, action: PayloadAction<{ groupId: string | number; product: IProduct }>) => {
    ...
    removeProduct: (
      state,
      action: PayloadAction<{ groupId: string | number; productId: string | number }>,
    ) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchProducts.fulfilled, (state, action: PayloadAction<IGroup[]>) => {
...
export const { addGroup, removeGroup, addProduct, removeProduct } = productGroupSlice.actions;
